...
DVT = DVT[,c(1,2,3,4,5,7,9,10,11,14)]

colnames(DVT) = c("chr","pos","other_allele","effect_allele","SNP","pval","beta","se","eaf","samplesize")
...
ME<- fread("GCST90200771_buildGRCh38.tsv.gz.txt",header=T)
...
res
write.csv(res, file="多变量结果200771.csv")
res_OR<-generate_odds_ratios(res$result)
write.csv(res_OR, file="多变量结果OR200771.csv")
res_OR
