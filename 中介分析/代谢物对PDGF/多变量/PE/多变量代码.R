...
PE = PE[,c(1,2,3,4,5,7,9,10,11,14)]

colnames(PE) = c("chr","pos","other_allele","effect_allele","SNP","pval","beta","se","eaf","samplesize")

PE$pval <- as.numeric(PE$pval)

PE$beta <- as.numeric(PE$beta)
...
#暴露2-HDL-----
rm(ME)
ME<- fread("GCST90200082_buildGRCh38.tsv.gz.txt",header=T)
...
res
write.csv(res, file="多变量结果200082.csv")
res_OR<-generate_odds_ratios(res$result)
write.csv(res_OR, file="多变量结果OR200082.csv")
res_OR
